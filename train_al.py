...
def parse_option():
    ...
    parser.add_argument("--num-samples", type=int, default=5500, help="num of samples per batch per worker")
    ...
    parser.add_argument("--dgl_file", type=str, default="/data/syf/gcc_modified/add_all_ordered.bin",
                        help="dataset bin for input")
    parser.add_argument("--gap", type=int, default=1, help="period 𝐹 of graph selection")
    parser.add_argument("--graph", type=int, default=11, help="the budget of input graphs")
    ...
    parser.add_argument("--random", type=int, default=0, help="random choice(default: false)")
    parser.add_argument("--init", type=int, default=20, help="number of epochs for initialization")
    # parser.add_argument("--dgl_file", type=str, default="/home/srtpgroup/gcc_modified/single_hic.bin")
    # dataset definition
    ...
    # GPU setting
    parser.add_argument("--gpu", default=3, type=int, nargs='+', help="GPU id to use.")
...
def train_moco(
    ...
    for idx, batch in enumerate(train_loader):
        ...
        if regular:
            ...
            # print(ewc_loss)
            # t1 = time.time()
            # print("one time", t1 - t0)
            loss = criterion(out) + opt.ewc_rate * ewc_loss
        else:
            loss = criterion(out)
        loss.backward()
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        if epoch % args.gap == 0 or epoch == 1:
            if epoch > args.init or epoch == 1:
                if len(train_dataset.jobs[0]) <= args.graph:
                    previous_model = load_model(args, epoch - 1)
                    train_dataset.device = torch.device(args.gpu)
                    train_dataset.update(previous_model, contrast, epoch, args.random)
                    train_loader = torch.utils.data.DataLoader(
                        dataset=train_dataset,
                        batch_size=args.batch_size,
                        collate_fn=labeled_batcher() if args.finetune else batcher(),
                ...
                if args.regular == 1:
                    regular = True
                print("Using regular:", True if args.regular else False)
                print("Using random:", True if args.random else False)
        if args.finetune:
