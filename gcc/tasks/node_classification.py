...
class NodeClassification(object):
    ...
    def __init__(self, dataset, model, hidden_size, num_shuffle, seed, **model_args):
        ...
        # self.data = create_node_classification_dataset(dataset, flip=True, rate=0.1, train_flag=True).data
    ...
    def _evaluate(self, features_matrix, label_matrix, num_shuffle):
        ...
        for train_idx, test_idx in idx_list:
            ...
            # roc = roc_auc_score(y_test, preds.toarray())
            f5.append(acc)
            # f5.append(recall)
            # f5.append(precision)
            f5.append(result)
            ...
            # all_results["rec"].append(recall)
            # all_results["pre"].append(precision)
            all_results["res"].append(result)
        for keys in all_results.keys():
            print(keys)
            value = sum(all_results[keys]) / len(all_results[keys])
            print("mean", value)
            std_value = np.std(np.array(all_results[keys]))
            print("std", std_value)
        return dict(
            (
